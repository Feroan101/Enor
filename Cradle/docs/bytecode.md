...
| Instruction | Opcode | Immediate Operand(s) | Total Size | Effect | pointer |
|------------|--------|--------------------|------------|--------| ----- |
| **NOP** | `0x00` | None | 1 byte | No operation |IP += 1 |
| **HALT** | `0xFF` | None | 1 byte | Ends program execution | no effect
...
| **JMP** | `0x08` | 2 bytes (absolute target) | 3 bytes | Set IP = target | IP = target |
...
| **JNZ** | `0x10` | 2 bytes (absolute target) | 3 bytes | Jump if top of stack is not 0 | IP = target |
